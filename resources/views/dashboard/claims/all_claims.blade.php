							...
							<tbody>
									...
									@foreach ($claims as $claim)
										...
										<tr>
											<td>{{ $claim->id }}</td>
											<td>{{$claim->icon}}</td>
											<td>{!! html_entity_decode(Str::limit($claim->claim,25)) !!}</td>
											...
											<td>
													<div class="btn-group" role="group" aria-label="Basic example">
																							{{-- <a type="button" href="{{ url('/dashboard/notice/edit') }}/{{ $claim->id }}" class="btn btn-secondary pd-x-25 active"><i class="icon ion-edit"></i></a> --}}
