						...
						<table id="datatable1" class="table display responsive nowrap">
							...
							<tbody>
									...
									@foreach ($events as $event)
										...
										<tr>
											<td>{{ $event->id }}</td>
											<td>{!! html_entity_decode(Str::limit($event->event_title,25)) !!}</td>
											<td>{!! html_entity_decode(Str::limit($event->event_details,25)) !!}</td>
											<td>
												<img src="{{ asset('uploads/events') }}/{{ $event->photo }}" alt="">
											</td>
											<td>
