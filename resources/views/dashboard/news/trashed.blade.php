...
@section('content')
      ...
      <div class="br-mainpanel">
        ...
        <!--               <a class="breadcrumb-item" href="#">Dashboard</a>-->
                   <span class="breadcrumb-item active">TRASHED NEWS</span>
                </nav>
             </div>
        ...
        <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
          <h4 class="tx-gray-800 mg-b-5">TRASHED NEWS</h4>
        </div>
					...
					<div class="table-wrapper">
						<table id="datatable1" class="table display responsive nowrap">
							...
							<tbody>
									...
									@foreach ($trashed_newses as $trashed_news)

										<tr>
											<td>{{ $trashed_news->id }}</td>
											<td>{!! html_entity_decode(Str::limit($trashed_news->news_title,25)) !!}</td>
											<td>{!! html_entity_decode(Str::limit($trashed_news->news_details,25)) !!}</td>
											<td>
												<img src="{{ asset('uploads/news') }}/{{ $trashed_news->photo }}" alt="">
											</td>

											<td>
													<div class="btn-group" role="group" aria-label="Basic example">
																							<a type="button" href="{{ url('/dashboard/news/restore') }}/{{ $trashed_news->id }}" class="btn btn-success pd-x-25 active"><i class="icon ion-loop"></i></a>
																							<a type="button" href="{{ url('/dashboard/news/force/delete') }}/{{ $trashed_news->id }}" class="btn btn-danger pd-x-25"><i class="icon ion-trash-a"></i></a>
																						</div>
											</td>
										</tr>
