					...
					<div class="table-wrapper">
						<table id="datatable1" class="table display responsive nowrap">
							...
							<tbody>
									...
									@foreach ($trashed_notices as $trashed_notice)

										<tr>
											<td>{{ $trashed_notice->id }}</td>
											<td>{!! html_entity_decode(Str::limit($trashed_notice->notice_title,25)) !!}</td>
											<td>{!! html_entity_decode(Str::limit($trashed_notice->notice_details,25)) !!}</td>
											<td>
												<img src="{{ asset('uploads/notices') }}/{{ $trashed_notice->photo }}" alt="">
											</td>

											<td>
													<div class="btn-group" role="group" aria-label="Basic example">
																							<a type="button" href="{{ url('/dashboard/notice/restore') }}/{{ $trashed_notice->id }}" class="btn btn-success pd-x-25 active"><i class="icon ion-loop"></i></a>
																							<a type="button" href="{{ url('/dashboard/notice/force/delete') }}/{{ $trashed_notice->id }}" class="btn btn-danger pd-x-25"><i class="icon ion-trash-a"></i></a>
																						</div>
											</td>
										</tr>
