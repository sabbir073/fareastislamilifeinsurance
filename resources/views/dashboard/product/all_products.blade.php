...
@section('content')
      ...
      <div class="br-mainpanel">
        ...
        <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
          <h4 class="tx-gray-800 mg-b-5">ALL PRODUCT</h4>
        </div>
          ...
          <div class="br-section-wrapper">
            ...
            <div class="table-wrapper">
                            ...
                            @foreach ($products as $product)
                              ...
                              <tr>
                                <td>{{ $product->id }}</td>
                                <td>{!! html_entity_decode($product->product_title) !!}</td>
                                <td>{!! html_entity_decode( Str::limit($product->product_details,20)) !!}</td>
                                <td>
                                  <img src="{{ asset('uploads/products') }}/{{ $product->product_photo }}" alt="">
                                </td>
                                <td>{{ $product->created_at->diffForHumans() }}</td>

                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                      <a type="button" href="{{ url('/dashboard/product/edit') }}/{{ $product->id }}" class="btn btn-secondary pd-x-25 active"><i class="icon ion-edit"></i></a>
                                      <a type="button" href="{{ url('/dashboard/product/delete') }}/{{ $product->id }}" class="btn btn-danger pd-x-25"><i class="icon ion-trash-a"></i></a>
                                    </div>
                                </td>
                              </tr>
